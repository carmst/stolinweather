...
function signalCopy(row) {
  ...
  if (row.recommendedSide) return `Model currently prefers ${row.recommendedSide.toUpperCase()}.`;
  ...
  return `
    <tr class="group hover:bg-white/5 transition-colors">
      <td class="px-8 py-6">
        <div class="flex items-center gap-4">
          ...
          <div>
            ...
            <span class="text-xs text-outline font-label">${row.ticker || row.contractSubtitle || row.eventDate || ""}</span>
          </div>
        </div>
      </td>
      <td class="px-6 py-6">
      ...
      <td class="px-8 py-6 text-right">
        <a class="bg-primary/10 hover:bg-primary text-primary hover:text-on-primary-container px-4 py-2 rounded-xl text-xs font-label font-bold transition-all duration-300 transform active:scale-95 inline-block" href="${row.inspectUrl}" target="_blank" rel="noreferrer">
          View on Kalshi
        </a>
      </td>
    </tr>
  ...
  root.innerHTML = `
      ...
      <div class="flex items-end justify-between">
        <div>
          ...
          <p class="text-on-surface-variant mt-4 max-w-xl font-body text-lg leading-relaxed">
            Tracking our manually curated contracts, model probability, and EV direction.
          </p>
        </div>
    ...
    <div class="mb-8 flex flex-wrap gap-4 items-center justify-between">
      <div class="flex gap-2">
        ...
        <button class="hover:bg-surface-container-high px-6 py-2 rounded-full text-sm font-label font-medium text-on-surface-variant transition-colors">Manual List</button>
      </div>
    ...
    <div class="glass-panel rounded-2xl overflow-hidden shadow-2xl">
        ...
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-surface-container-low/50">
              ...
              <th class="px-6 py-5 text-[10px] font-label font-bold uppercase tracking-widest text-outline text-center">Stolin Prediction</th>
          ...
          <tbody class="divide-y divide-white/5">
            ${rows.length ? rows.map(buildWatchlistRow).join("") : `
              <tr>
                <td class="px-8 py-12 text-slate-400" colspan="5">No curated contracts are configured yet. Edit config/watchlist.json to choose the locations or tickers we want to monitor.</td>
              </tr>
            `}
          </tbody>
        </table>
      ...
      <div class="p-8 bg-surface-container-low/30 flex justify-between items-center border-t border-white/5">
        <span class="text-xs font-label text-outline">Showing ${rows.length} manually watched contracts</span>
      ...
      <div class="glass-panel p-8 rounded-3xl col-span-1 md:col-span-2 relative overflow-hidden group">
        <div class="relative z-10">
          ...
          <p class="text-on-surface-variant font-body mb-6 leading-relaxed">
            We are watching the contracts you curated manually. The EV column shows the current model edge and whether the setup is positive, negative, or flat.
          </p>
